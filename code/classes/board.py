...
class Board():
    ...
    def create_board(self, source_file):
        dimension=int(source_file[-7]) 																		#definieer dimensie op basis van bestandsnaam
        self.boarddummy=np.zeros((dimension+2,dimension+2), int).astype(str) 									#Creeer een array met nullen op basis van dimensie om vol te zetten met auto's
    ...
    def load_board(self, y):
        ...
        for key,row in cars.items():																                        #loop door input dataframe
            posx= 7-cars[key][2]
            posy= cars[key][1]
            board[posx][posy]=board[posx][posy].replace('0', key)						                    #Vervang 0 met de relevante auto letter
            if cars[key][0]=='H':																	        #check of de auto horizontaal of verticaal is georienteerd om te bepalen waar de volgende letter moet komen
                ...
                if cars[key][3]==3:																		    #check hoe groot de auto is om te bepalen of er nog een derde letter bij moet komen
